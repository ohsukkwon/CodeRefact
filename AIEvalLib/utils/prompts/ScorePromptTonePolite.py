...
class ScorePromptTonePolite(IScorePrompt):
    ...
    def get_my_keyname(self):
        return PROMPT_KEY_NAME_TONEPOLITE
    ...
    def get_my_csvname(self):
        ...
        return f'sentences_{PROMPT_KEY_NAME_TONEPOLITE}.csv'
    ...
    def get_noti_role_prompt(self):
        ...
        return \
            '''Act as a 40-year-old male {innerTgtLanguage} teacher.
지금부터 학생이 제출한 polite 톤변경 문장이 올바른지 채점을 할 것이며, 판단기준을 설명할께.

#채점 주요기준 7가지
1) 주어진 output 문장에 철자가 틀린 단어가 있으면, 20점보다 낮은 점수로 판단할것.
2) 주어진 output 문장이 {innerTgtLanguage} 에 맞게 완벽하게 polite 톤변경되어 있지 않으면, 20점보다 낮은 점수로 판단할것.
...
| input | output |
| ----- | ------ |
| 아쉽지만 여행 때문에 어쩔 수 없어. | 저도 아쉽지만, 여행 때문에 어쩔 수가 없습니다. |
| 우리 거기서 연극 보기로 약속했잖아. 왜 갑자기 장소를 바꾸려고 해? | 우리 거기서 연극 보기로 약속했잖아요. 왜 갑자기 장소를 바꾸려고 하세요? |
...

채점할 준비가 되면 알려줘. 그러면 채점이 필요한 문장을 알려줄께.''' \
    ...
    def get_do_score_prompt(self):
        ...
        return \
'''#지시사항
아래에 제시된 문장들은 input 문장을 output 문장으로 polite 톤변경한 결과야.
...
아래 주어진 {innerSentenceCount}개의 {innerSrcLanguage} 문장을 polite 톤으로 변경해줘.
