...
class ScorePromptToneSocial(IScorePrompt):
    ...
    def get_my_keyname(self):
        return PROMPT_KEY_NAME_TONESOCIAL
    ...
    def get_my_csvname(self):
        ...
        return f'sentences_{PROMPT_KEY_NAME_TONESOCIAL}.csv'
...
당신은 학생이 제출한 Social톤 변경 문장이 올바른지 채점을 할것이며, 판단기준을 설명할께.
...
2) 주어진 output 문장이 {innerTgtLanguage} 에 맞게 완벽하게 번역되어 있지 않으면, 20점보다 낮은 점수로 판단할것.
3) 주어진 output 문장이 {innerTgtLanguage} 문법에 맞지 않으면, 20점보다 낮은 점수로 판단할것.
4) Social톤 변경후 input 문장의 의미가 output 문장에서 의미가 변경되었다면, 20점보다 낮은 점수로 판단할것.
...
| input | output |
| ----- | ------ |
| 저도 아쉽지만, 여행 때문에 어쩔 수가 없습니다. |	저도 아쉽지만, 여행 때문에 어쩔 수가 없습니다. ✈️ #여행 #아쉬움 #어쩔수없음 |
| 우리 거기서 연극 보기로 약속했잖아요. 왜 갑자기 장소를 바꾸려고 하세요?  |  우리 거기서 연극 보기로 약속했잖아요. 왜 갑자기 장소를 바꾸려고 하세요? 😤 #연극 #약속 #장소변경 |
''' \
...
아래 주어진 {innerSentenceCount}개의 {innerSrcLanguage} 문장을  Social 톤문장으로 변경해줘.
