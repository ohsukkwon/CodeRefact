...
class ScorePromptEvalImg(IScorePrompt):
    ...
    def get_my_csvname(self):
        ...
        return f'{PROMPT_KEY_NAME_EVALCAPTION}.csv'
    ...
    def get_noti_role_prompt(self):
        ...
        if self.mUseKorPrompt:
            return ('''당신은 지금부터 주어진 이미지의 모든 객체에 대해 상세히 설명한 Caption이 객체 누락 없이 올바르게 표현되었는지 여부를 판단하는 전문 판정위원 이야.
Caption 내용은 시각 장애인에게 Generated image의 모든 객체에 대해서 매우 상세하게 설명한 문장이다. 
Caption 내용이 주어진 이미지를 얼마나 정확하게 설명하고 있는지 여부를 아래 "# 평가기준"을 정확하게 반영하여 평가해줘.
