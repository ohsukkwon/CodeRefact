...
class ScorePromptCorrection(IScorePrompt):
    ...
    def get_my_keyname(self):
        return PROMPT_KEY_NAME_CORRECT
    ...
    def get_my_csvname(self):
        ...
        return f'sentences_{PROMPT_KEY_NAME_CORRECT}.csv'
...
당신은 학생이 제출한 교정 문장이 올바른지 채점을 할것이며, 판단기준을 설명할께.
...
2) 주어진 output 문장이 {innerTgtLanguage} 로 완벽하게 교정되어 있지 않으면, 20점보다 낮은 점수로 판단할것.
3) 주어진 output 문장이 {innerTgtLanguage} 문법에 맞지 않으면, 20점보다 낮은 점수로 판단할것.
4) 교정후 input 문장의 의미가 output 문장에서 의미가 변경되었다면, 20점보다 낮은 점수로 판단할것.
...
평가의 정확도를 높이기 위해 더 자연스러운 문장을 제시해 줄테니, 참조해서 평가해줘.
#자연스러운 문장 예시 1가지
| 어재 너므 바빠서 허루종일 밥을 못 먹엇다  |  어제 너무 바빠서 하루종일 밥을 못 먹었다  |
''' \
        .format( innerTgtLanguage=self.mArgs.getValInnerArg(ARG_KEY_NAME_TARGETLANGUAGE),
    ...
    def get_do_score_prompt(self):
        ...
        return \
'''당신은 아래의 채점이 필요한 문장 {innerSentencesCount}가지에 대해서 총30점 기준으로 평가하고, 점수와 reason을 알려주고, reason 은 반드시 {innerMotherLanguage}으로 표현해줘.
...
''' \
    ...
    def get_sentence_template_prompt(self):
        ...
        return \
        '''From now act as a 40-year-old male {innerTgtLanguage} teacher.
아래 주어진 {innerSentenceCount}개의 {innerTgtLanguage} 문장을 {innerTgtLanguage} 문법에 정확하게 일치하도록 교정해줘.
...
#교정이 필요한 문장 {innerSentenceCount}가지
''' \
