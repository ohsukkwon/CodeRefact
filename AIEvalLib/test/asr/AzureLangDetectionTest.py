...
from GlobalUtil import is_azure_support_langcode
from MSA_LanguageDetector import get_client_ai_language_detection_msa

__test_sample_arr = [
    ["Afrikaans","EMPTY","af","empty",	"Wanneer ek na die rooi sonsondergang kyk, voel dit asof ek teruggaan na my kinderdae toe ek onskuldig gespeel het. Ek het 'n oomblik geneem om aan my kinderdae te dink. Dit was wonderlik om saam 'n goeie tyd te hê. Ek is opreg dankbaar."],
...
def language_detection_example(client):
    ...
    for item_inner in __test_sample_arr:
        print(item_inner)
        documents.append({ "id" : ___tmp_idx, "text": item_inner[4]})
        ___tmp_idx += 1
        if ___tmp_idx >= 5:
            break


    response_arr_total = client.detect_language(documents = documents, country_hint = 'us')
    ...
    for _inner_idx, one_item in enumerate(response_arr_total):
        ...
        print(f"response[{_inner_idx}]:{response}")
        language_name = response.primary_language.name
        print(f"Language:{language_name}")
        iso6391_lang_code = response.primary_language.iso6391_name
        print(f"iso6391_name:{iso6391_lang_code}")

        if is_azure_support_langcode(iso6391_lang_code):
            print(f"♥♥♥ iso6391_name[{iso6391_lang_code}] is SUPPORT♥♥♥")
        else:
            print(f"!!! OMG! iso6391_name[{iso6391_lang_code}] is not supported!!!{one_item}")

        __test_sample_arr[_inner_idx][1] = language_name
        __test_sample_arr[_inner_idx][3] = iso6391_lang_code

        if __test_sample_arr[_inner_idx][2] == __test_sample_arr[_inner_idx][3]:
            # if __test_sample_arr[_inner_idx][0] == __test_sample_arr[_inner_idx][1]:
            #     pass
            # else:
            #     documents_failed.append(__test_sample_arr[_inner_idx])
                pass
        else:
            documents_failed.append(__test_sample_arr[_inner_idx])
            pass
